...
public class OverviewTest extends TestCase {
    ...
    public void testAsStringMartin1() {
        ...
        try {
            ...
            Document testdoc = new Document(new File(
            "testdata/trainingslist_20060101_20060331.xml"));
            Trainer martin = (Trainer) testdoc.findBeanByQuery(
            "org.rapidbeans.clubadmin.domain.Trainer[id = 'Blümel_Martin_']");
            ArrayList<Trainer> trainers = new ArrayList<Trainer>();
            trainers.add(martin);
            Department aikido = (Department) testdoc.findBeanByQuery(
            "org.rapidbeans.clubadmin.domain.Department[id = 'Budo-Club Ismaning/Aikido']");
            ArrayList<Department> departments = new ArrayList<Department>();
            departments.add(aikido);
            RapidBeansLocale locale = new RapidBeansLocale("de");
            locale.init("org.rapidbeans.clubadmin");
            for (RapidBean bean : testdoc.findBeansByType(
            ...
            assertEquals("Trainingsübersicht   Trainer: Blümel, Martin,   Abteilung: Budo-Club Ismaning/Aikido", st.nextElement());
            assertEquals("---------------------------------------------------------------------------", st.nextElement());
            String test1 = (String) st.nextElement();
            assertEquals("  1. 02.01.2006 Montag     20:30 Meditation II                    20,00 EUR",
                    test1);
        } finally {
    ...
    public void testAsStringMartin2() {
        ...
        try {
            Locale.setDefault(Locale.GERMANY);
            Document testdoc = new Document(new File(
            "testdata/trainingslist_20070901_20071231.xml"));
            Trainer martin = (Trainer) testdoc.findBeanByQuery(
            "org.rapidbeans.clubadmin.domain.Trainer[id = 'Blümel_Martin_']");
            ArrayList<Trainer> trainers = new ArrayList<Trainer>();
            trainers.add(martin);
            ...
            assertEquals("Trainingsübersicht   Trainer: Blümel, Martin,   Abteilung: Budo-Club Ismaning/Aikido", st.nextElement());
            assertEquals("---------------------------------------------------------------------------", st.nextElement());
            assertEquals("  1. 06.09.2007 Donnerstag 20:00 Aikido Erwachsene                20,50 EUR", st.nextElement());
        } finally {
            Locale.setDefault(formerDefault);
        }
    }
    ...
    public void testAsStringReinhard() {
        ...
        try {
            ...
            "testdata/trainingslist_20070901_20071231.xml"));
            Trainer martin = (Trainer) testdoc.findBeanByQuery(
            "org.rapidbeans.clubadmin.domain.Trainer[id = 'Landsberger_Reinhard_']");
            ArrayList<Trainer> trainers = new ArrayList<Trainer>();
            trainers.add(martin);
            Department aikido = (Department) testdoc.findBeanByQuery(
            "org.rapidbeans.clubadmin.domain.Department[id = 'Budo-Club Ismaning/Aikido']");
            ...
            String overview = Overview.asString(trainers, departments, locale);
            StringTokenizer st = new StringTokenizer(overview, "\n");
            assertEquals("Trainingsübersicht   Trainer: Landsberger, Reinhard,   Abteilung: Budo-Club Ismaning/Aikido", st.nextElement());
            assertEquals("---------------------------------------------------------------------------", st.nextElement());
            assertEquals("  1. 04.09.2007 Dienstag   18:00 Aikido Kinder/Erwachsene         15,50 EUR", st.nextElement());
        } finally {
            Locale.setDefault(formerDefault);
        }
    }
