...
public class EvidenceTest extends TestCase {
    ...
    /**
     * Test method for {@link org.rapidbeans.clubadmin.domain.report.Evidence#insertEvidence(java.lang.String, org.rapidbeans.clubadmin.domain.Trainer, org.rapidbeans.clubadmin.domain.Department, java.util.List)}.
     * @throws IOException if IO fails
     */
    public void testReadTemplate() throws IOException {
        Template tmpl = Evidence.readTemplate(
                new File("testdata/testevidence/templateEvidence.rtf"));
        ...
        out.close();
        assertTrue(FileHelper.filesEqual(testfile,
                new File("testdata/testevidence/templateEvidence1x2.rtf"),
                true, true));
    ...
    public void testDetermineRowCount32() throws IOException {
        Template tmpl = Evidence.readTemplate(
                new File("testdata/testevidence/templateEvidence32.rtf"));
    ...
    public void testReportDepartmentSimple() throws IOException {
        ...
        Trainer trainer = (Trainer) testdoc.findBean(Trainer.class.getName(), "Blümel_Ulrike_");
        ...
        assertTrue("Test file " + testfile.getAbsolutePath()
                + " does not equal reference file "
                + expected.getAbsolutePath(),
                FileHelper.filesEqual(testfile, expected, true, true));
        testfile.delete();
    ...
    public void testReportOverflowDepartmentSingle() throws IOException {
        ...
        ApplicationManager.start(app);
        Document testdoc = new Document(new File("testdata/testevidence/trainingsDataAikido.xml"));
        Trainer trainer = (Trainer) testdoc.findBean(Trainer.class.getName(), "Blümel_Martin_");
        Department department = (Department) testdoc.findBean(Department.class.getName(), "Budo-Club Ismaning/Aikido");
        File testfile = new File("testdata/testevidence/testreport.rtf");
        File expected = new File("testdata/testevidence/resultEvidenceOverflowDepartmentSingle.rtf");
        File template = new File("testdata/testevidence/templateEvidence.rtf");
        Evidence.printReportEvidence(testfile, template, department, trainer, true);
//        Runtime.getRuntime().exec("cmd.exe /C " + testfile.getAbsolutePath());
...
//        Runtime.getRuntime().exec("cmd.exe /C " + testfile.getAbsolutePath());
    ...
    public void testReportOverflowDepartmentsCoherent() throws IOException {
        ApplicationMock app = new ApplicationMock();
        ApplicationManager.start(app);
        String trainerId = "Blümel_Martin_";
