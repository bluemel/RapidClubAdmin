...
public abstract class Training extends RapidBeanBaseTraining {
        ...
        if (time != null) {
            ...
            if (timeend != null) {
                if (time.compareTo(timeend) == 1) {
                    ...
                    throw new ValidationException("invalid.prop.trainingdate.start.greater.end",
                            this,
                            "invalid value \"" + time.toString()
                            + "\" for property \"timestart\""
                            + " greater than property \"timeend\" = \""
                            + timeend.toString() + "\"", args);
                }
                switch (time.compareTo(timeend)) {
                case 0:
                    final Object[] args1 = {time, timeend };
                    throw new ValidationException("invalid.prop.trainingdate.start.equals.end",
                            this,
                            "invalid value \"" + time.toString()
                            + "\" for property \"timestart\""
                            + " equals property \"timeendt\" = \""
                            + timeend.toString() + "\"", args1);
                case 1:
            ...
            if (timestart != null) {
                ...
                case 0:
                    ...
                    throw new ValidationException("invalid.prop.trainingdate.start.equals.end",
                        this,
                        "invalid value \"" + time.toString()
                        + "\" for property \"timeend\""
                        + " equals property \"timestart\" = \""
                        + timestart.toString() + "\"", args1);
                case -1:
                    final Object[] args2 = {time, timestart };
                    throw new ValidationException("invalid.prop.trainingdate.end.less.start",
                        this,
                        "invalid value \"" + time.toString()
                        + "\" for property \"timeend\""
                        + " less than property \"timestart\" = \""
                        + timestart.toString() + "\"", args2);
                default:
                    break;
                }
            }
            ...
            for (int i = 0; i < trhbtsSize; i++) {
                for (int j = i + 1; j < trhbtsSize; j++) {
                    ...
                    case 0:
                        break;
                    default:
                        throw new RapidBeansRuntimeException("unexpected compare result "
                                + trhbtsSorted.get(i).compareValues(trhbtsSorted.get(j)));
                    }
                }
            }
        }
    ...
    /**
     * A Training that has been "closed" or "cancelled" or "checkedbefore"
     * is reset. So it will become either "asplanned" or "modified"
     */
    ...
    public void toggleCancelled() {
        ...
        case checked:
            throw new RapidClubAdminBusinessLogicException(
                    "training.invalid.state.transition.checked.cancelled",
        ...
        case closed:
            throw new RapidClubAdminBusinessLogicException(
                    "training.invalid.state.transition.closed.cancelled",
                    "Trainings with closed location can't be set to cancelled");
        default:
            throw new RapidBeansRuntimeException("Unexpected TrainigState"
                    + ", order = " + this.getState().ordinal());
        }
    }
    ...
    public void toggleChecked() {
        ...
        case cancelled:
            throw new RapidClubAdminBusinessLogicException(
                    "training.invalid.state.transition.cancelled.checked",
                    "Cancelled Trainings can't be checked or unchecked."
                    + "Uncancel them before");
        case closed:
            throw new RapidClubAdminBusinessLogicException(
                    "training.invalid.state.transition.closed.checked",
                    "Trainings with closed location can't be set to cancelled");
        default:
            throw new RapidBeansRuntimeException("Unexpected TrainigState"
                    + ", order = " + this.getState().ordinal());
    ...
    private void markChecked() {
        ...
        if (trlist != null) {
            ClubadminUser user = (ClubadminUser) trlist.getContainer().findBean(
                    ClubadminUser.class.getName(), currentUser.getIdString());
            if (user == null) {
    ...
    public void validate() {
            ...
            for (final TrainingHeldByTrainer trhbt : this.getHeldbytrainers()) {
                if (trhbt.getRole() == null) {
                    throw new RapidClubAdminBusinessLogicException(
                            ...
                            "Trainings held by trainers must have a trainer role set.",
                            new Object[] {trhbt});
                }
                trhbt.validate();
            }
        ...
        if (!checkFutureChangeOk()) {
            throw new RapidClubAdminBusinessLogicException(
                    "invalid.training.state.transition.checked.future",
    ...
    public boolean checkFutureChangeOk() {
        ...
        if (this.getState() == TrainingState.checked
                ...
                && this.getDate().compareTo(new Date()) > 0
    ...
    public void checkChangingAllowed() {
        if (!checkChangingAllowedOk()) {
            throw new RapidClubAdminBusinessLogicException(
                    "invalid.training.state.transition.checked.allowed",
    ...
    public boolean checkChangingAllowedOk() {
        ...
        default:
            throw new RapidBeansRuntimeException(
                    "Unexpected state \"" + this.getState().name() + "\"");
        }
    }
    ...
    public Time getTimeWorked(final UnitTime unitTime) {
        return (Time) new Time(
                this.getTimeend().getMagnitude().subtract(
                    this.getTimestart().getMagnitude()),
