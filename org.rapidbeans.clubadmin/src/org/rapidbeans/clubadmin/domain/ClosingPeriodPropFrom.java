...
public class ClosingPeriodPropFrom extends PropertyDate {
    ...
    public Date validate(final Object newValue) {
        ...
        if (toTime != null) {
            if (newDate.getTime() > toTime.getTime()) {
                final PropertyDate newProp = (PropertyDate)
                Property.createInstance(this.getType(), null);
                newProp.setValue(newValue);
                final Object[] args = {newProp, this.getBean().getProperty("to")};
                throw new ValidationException(
                        "invalid.prop.closingperiod.from.greater.to",
                        this.getBean(),
                        "invalid value \"" + newDate.toString()
                        + "\" for property \"from\""
                        + " greater than property \"to\" = \""
                        + toTime.toString() + "\"", args);
            }
        }
        return newDate;
    }
     ...
     * react on changes.
     * Accepts the following datatypes:<br/>
