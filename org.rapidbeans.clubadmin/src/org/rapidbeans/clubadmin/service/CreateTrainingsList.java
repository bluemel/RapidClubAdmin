...
public class CreateTrainingsList extends Action {
    ...
    public void execute() {
        try {
            ...
            final DocumentView docview = ApplicationManager.getApplication().openDocumentView(doc, "trainingslist", "expert");
    ...
    /**
     * create a TrainingsList document out of a given
    ...
    public static Document createNewTrainingsList(
            final Document srcMasterDoc,
            final String depId) {
        Department department = null;
        if (depId != null && depId.length() > 0) {
            department = (Department) srcMasterDoc.findBean(
                    "org.rapidbeans.clubadmin.domain.Department", depId);
        }
        ...
        if (department == null) {
            ...
            for (PropertyAssociationend srcColProp : srcRootMd.getColPropertiesComposition()) {
                ...
                final PropertyAssociationend tgtProp = (PropertyAssociationend) targetRootBp.getProperty(propName);
                if (tgtProp != null && (
                        propName.equals("sports")
                        || propName.equals("salaryComponentTypes")
                        || propName.equals("trainerattributes")
                        || propName.equals("locations")
                    ...
                    if (propName.equals("trainerroles")) {
                        for (final Object o1 :  (Collection<?>) srcColProp.getValue()) {
                            ...
                            for (PropertyAssociationend srcColProp1 : srcTr.getColPropertiesComposition()) {
                                ...
                                final PropertyAssociationend tgtProp1 = (PropertyAssociationend) clonedTr.getProperty(propName1);
                                            ...
                                            if (person.getDepartments().contains(department)) {
                                                ...
                                                if (ps.getComponents() != null) {
                                                    for (SalaryComponent sc : ps.getComponents()) {
                                                        ...
                                                        ((PropertyCollection) clonedSc.getProperty("salaryComponentType")).addLink(
                                                                new LinkFrozen(sc.getSalaryComponentType().getIdString()),
                                                                false, false, false);
                            ...
                            if (srcClub.getDepartments().contains(department)) {
                                ...
                                for (final Department srcDep : srcClub.getDepartments()) {
                                    if (srcDep.equals(department)) {
                                        final Department clonedDep = new Department(new String[]{department.getName()});
                        ...
                        for (final Object o1 : (Collection<?>) srcColProp.getValue()) {
                            ...
                            if (srcTrainer.getDepartments() != null && srcTrainer.getDepartments().contains(department)) {
                                Trainer clonedTrainer = new Trainer(
                                        new String[]{srcTrainer.getLastname(),
                                                srcTrainer.getFirstname(),
                                                srcTrainer.getMiddlename()});
            ...
            for (final Trainer trainer : targetRootBp.getTrainers()) {
                ...
                if (trpProps != null) {
                    ...
                    for (final Link trplink : trpProps) {
                        final TrainerPlanning srcTrp = (TrainerPlanning) srcMasterDoc.findBean(
                                "org.rapidbeans.clubadmin.domain.TrainerPlanning", trplink.getIdString());
                ...
                final List<Link> depProps = (List<Link>)
                trainer.getProperty("departments").getValue();
                if (depProps != null) {
