...
public class DocumentViewBillingPeriod extends DocumentViewSwing {
     ...
     */
    public DocumentViewBillingPeriod(final Application client, final Document doc,
            final String docconfname, final String viewconfname,
            final Filter filter) {
        super(client, doc, docconfname, viewconfname, filter);
        ((JInternalFrame) this.getWidget()).setTitle(ApplicationManager.
                getApplication().getCurrentLocale().getStringGui(
                        "view.masterdata.billingperiod.title"));
    }
}
