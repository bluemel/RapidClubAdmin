 ...
 * and open the editor to create a new TrainingSpecial
 * instance under the one and only department
 *
...
public class NewSpecialTraining extends Action {
    ...
    public void execute() {
        ...
        final DocumentView activeView =
            client.getMainwindow().getActiveDocumentView();
        if (activeView == null) {
            client.messageInfo("no document view active");
            return;
        }
        final DocumentView expertView = client.openDocumentView(
                activeView.getDocument(), "trainingslist", "expert");
