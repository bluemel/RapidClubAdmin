...
public class EditorBillingPeriod extends EditorBeanSwing {
    ...
    private Department[] getSelectedDepartmentsWithTrainingsLists() {
        ...
        for (int i = 0; i < selDepsSize; i++) {
            ...
            if (deps[i] == null) {
                throw new RapidBeansRuntimeException("departement \"" + selDep.getIdString()
                        + "\" not found over TrainingsList");
            }
        }
        return deps;
    }
     ...
     * 
     * @param confirm
     *            drives interactive confirmation
     * @param allDepartments
     *            if true training lists for all departments will be generated
     *            otherwise only for selected departments
     */
    private void generateTrainingslists(final boolean confirm, final boolean allDepartments) {
        ...
        if (confirm
                && (!app.messageYesNo(locale.getStringMessage("question.saveas.file.trainingslist.knowwhatyoudo"),
    ...
    private void printCollectedOverviewReport() {
        ...
        try {
            ...
            if (app.getSettingsRapidClubAdmin().getReportfolder() != null) {
                ...
                overviewFile = File.createTempFile("SammeldruckAbrechnungszeitraum_" + getBillingPeriod().getIdString()
                        + "_", ".txt");
            }
    ...
    private void printCollectedEvidenceReport() {
        try {
            ...
            if (app.getSettingsRapidClubAdmin().getReportfolder() != null) {
                ...
                evidenceFile = File.createTempFile("SammeldruckNachweiseAbrechnungszeitraum_"
                        + getBillingPeriod().getIdString() + "_", ".rtf");
            }
    ...
    class DepartmentListCellRenderer extends RendererListCollection {
         ...
         *            the index
         * @param isSelected
         *            flag that marks if the cell is selected
         * @param cellHasFocus
        ...
        public Component getListCellRendererComponent(final JList list, final Object value, final int index,
            ...
            if (docview != null) {
                ...
                switch (allTrState) {
                case all_asplanned:
                    icon = new ImageIcon(RapidClubAdmin.class.getResource("presentation/pictures/training0Default.gif"));
                    break;
                case one_checkormodified:
                    icon = new ImageIcon(RapidClubAdmin.class.getResource("presentation/pictures/training1InWork.gif"));
                    break;
                case all_checked:
                    icon = new ImageIcon(RapidClubAdmin.class.getResource("presentation/pictures/training2Checked.gif"));
                    break;
                default:
                    throw new RapidBeansRuntimeException("Unexpected state \"" + allTrState.name() + "\"");
                }
