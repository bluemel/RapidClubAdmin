...
public final class ViewOverview extends DocumentView {
    ...
    public ViewOverview(final Application client, final Document doc, final String docconfname,
        ...
        ImageIcon icon = ((MainWindowSwing) client.getMainwindow()).getIconManager().getIcon(
                "view.trainingslist.overview.icon");
        ...
        this.leftPanel.add(this.clubsList, new GridBagConstraints(0, 1, 1, 1, 1.0, 1.0, GridBagConstraints.NORTH,
                GridBagConstraints.NONE, new Insets(5, 5, 5, 5), 0, 0));
        this.leftPanel.add(this.departmentsLabel, new GridBagConstraints(0, 2, 1, 1, 1.0, 0.0,
    ...
    private void selectDefault() {
        ...
        if (wdId != null) {
            departmentToSelect = (Department) client.getMasterDoc().findBean(
                    "org.rapidbeans.clubadmin.domain.Department", wdId);
        }
        if (user.getIsalsotrainer() != null) {
            ...
            if (role == Role.DepartmentAdministrator && authDeps != null && authDeps.size() > 0) {
                ...
                final List<RapidBean> deps = this.getDocument().findBeansByType(
                        "org.rapidbeans.clubadmin.domain.Department");
                if (deps.size() > 0) {
     ...
     * 
     * @param departmentToSelect
     *            the Department to select
     */
     ...
     * 
     * @param trainerToSelect
     *            the Trainer to select
     */
    ...
    private void printReportEvidence() {
        ...
        try {
            final File evidenceFile = File.createTempFile(
                    "Nachweis_" + this.getSelectedTrainers().get(0).getLastname(), ".rtf");
            final File templateFile = Evidence.findEvidenceReportTemplateFile(false);

            // generate the evidence report
            Evidence.printReportEvidence(evidenceFile, templateFile, this.getSelectedDepartments().get(0), this
                    .getSelectedTrainers().get(0), true);
            ...
            if (PlatformHelper.getOsfamily() == OperatingSystemFamily.windows) {
                ...
                ApplicationManager.getApplication().messageInfo(
                        "Automatic Pop-Up not yet implemented for operating system\n\"" + PlatformHelper.getOsName()
