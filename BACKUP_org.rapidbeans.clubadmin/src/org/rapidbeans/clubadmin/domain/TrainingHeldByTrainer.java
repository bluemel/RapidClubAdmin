...
public class TrainingHeldByTrainer extends RapidBeanBaseTrainingHeldByTrainer {
     ...
     *
     * @return the salary or null if the TrainingHeldByTrainer
     *         if not defined properly
     */
    public AbstractSalary getSalary() {
        if (this.getTrainer() != null) {
            if (this.getRole() == null) {
                throw new RapidClubAdminBusinessLogicException("trhbt.null.role", "Null role for TrainingHeldByTrainer \""
                        + this.getIdString() + "\" for Training \""
    ...
    public Money getMoneyEarned() {
        ...
        final BigDecimal minutesWorked = minutesEnd.subtract(minutesStart);
