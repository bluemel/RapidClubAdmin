...
public class TrainingsList extends RapidBeanBaseTrainingsList {
    ...
    public AllTrainingState getTrainingsState() {
        ...
        for (TrainingRegular tr : this.getTrainings()) {
            ...
            case closed:
                break;
            default:
                throw new RapidBeansRuntimeException("Unexpected state \""
                        + allTrainingsState.name() + "\"");
            }
    ...
    public List<TrainingRegular> getTrainings() {
        ...
        for (RapidBean bean : this.getContainer().findBeansByType(
                TrainingRegular.class.getName())) {
     ...
     */
    private static final long MAX_PERIOD_FOR_TRAININGS_UPDATE =
        5L * 366 * 24 * 60 * 60 * 1000;

    /**
     * creates new trainings and removes unneccessary trainings
     * after from or to date of this trainings period has changed.
     *
    ...
    public void updateTrainings(final int mode, final Date newDate) {
        ...
        if (((dTo.getTime() - dFrom.getTime()) > MAX_PERIOD_FOR_TRAININGS_UPDATE)
            ...
            ApplicationManager.getApplication().messageInfo(ApplicationManager.getApplication().
                    getCurrentLocale().getStringMessage(
                            "info.bizrule.trainingslist.notrainigsupdate"));
            return;
        }
    ...
    public void updateTrainingsClosing() {
        ...
        for (RapidBean b : this.getContainer().findBeansByType(
     ...
     */
    private boolean trainingInOneOfCps(final TrainingRegular tr,
            final Collection<ClosingPeriod> cps) {
        ...
        for (ClosingPeriod cp : cps) {
            ...
            if (cpFrom.getTime() <= trdate.getTime()
                    && cpTo.getTime() >= trdate.getTime()) {
                return true;
            }
        }
        return false;
    ...
    /**
     * removes unneccessary closing periods and adds additional ones
     * from masterdata
    ...
    public void updateClosingPeriodsFromMasterdata() {
        ...
        if (client != null) {
            ...
            if (masterdata != null && masterdata.getClosingperiods() != null) {
                for (ClosingPeriod cpMaster : masterdata.getClosingperiods()) {
                    if (PropertyDate.dateIntervalsOverlap(cpMaster.getFrom(), cpMaster.getTo(),
                            this.getFrom(), this.getTo())
                            && (!this.getContainer().contains(cpMaster))) {
        ...
        if (this.getClosingperiods() != null) {
            for (ClosingPeriod cp : this.getClosingperiods()) {
                if (!PropertyDate.dateIntervalsOverlap(cp.getFrom(), cp.getTo(),
                        this.getFrom(), this.getTo())) {
     ...
     */
    protected List<RapidBean> createNewTrainings(
            final Date dFrom, final Date dTo) {

        final List<RapidBean> newTrs = new ArrayList<RapidBean>();
        final HashMap<Integer, Collection<TrainingDate>> trdates =
            getTrainingDatesAsGCIndexedHashMap();
     ...
     * @return collection with trainings to remove
     */
    protected Collection<RapidBean> findObsoleteTrainings(
            final Date dFrom, final Date dTo) {
        final Collection<RapidBean> obsTrainings = new ArrayList<RapidBean>();
        if (this.getContainer().findBeansByType(
                "org.rapidbeans.clubadmin.domain.Training").size() == 0) {
            return obsTrainings;
        }
    ...
    /**
     * @return a HashMap with all TrainingDates indexed by their day of week
     *         as GregorianCalendar defined int constant.
     */
    private HashMap<Integer, Collection<TrainingDate>>
        getTrainingDatesAsGCIndexedHashMap() {

        final HashMap<Integer, Collection<TrainingDate>> trdates
            = new HashMap<Integer, Collection<TrainingDate>>();
        ...
        for (RapidBean b : this.getContainer().findBeansByType(
        "org.rapidbeans.clubadmin.domain.TrainingDate")) {
    ...
    /**
     * create all trainings for one day and add them
     * to the given trainings collection.
     *
     * @param trainingsCreated the trainings to create
     * @param d the date
     * @param trdates all training dates
     */
    private void createTrainingsForDay(
            final Collection<RapidBean> trainingsCreated,
            final Date d, final Collection<TrainingDate> trdates) {
        TrainingRegular tr;
            ...
            if ((trdate.getTrainings() == null) || (!trdate.getTrainings().contains(tr))) {
                ...
                if (trdate.getTrainerplannings() != null) {
                    for (TrainerPlanning pl : trdate.getTrainerplannings()) {
                        ...
                        if (pl.getDefaulttrainers() == null
                                || pl.getDefaulttrainers().size() == 0) {
                            trhbt.setTrainer(null);
                        } else {
                            // take the first default trainer if any defined
                            trhbt.setPropValue("trainer",
                                    pl.getDefaulttrainers().iterator().next().getIdString());
                        }
                        if (pl.getRole() == null) {
                            trhbt.setRole(null);
                        } else {
