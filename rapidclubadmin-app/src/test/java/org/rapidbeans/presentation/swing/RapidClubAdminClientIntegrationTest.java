...
public class RapidClubAdminClientIntegrationTest {
	...
	public void testCreateTwoDuplicateTrainers() {
		...
		Assert.assertEquals("trainers",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(2).getLastPathComponent()).getColProp()
						.getType().getPropName());
	...
	public void testCreateTrainer() {
		...
		Assert.assertEquals("Bl" + Umlaut.L_UUML + "mel_Martin_",
	...
	/**
	 * create a duplicate trainer and verify that no tree node for the trainer is
	 * created.
	 */
	@Test
	public void testCreateTrainerDuplicate() {
		...
		Assert.assertEquals("trainers",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(2).getLastPathComponent()).getColProp()
						.getType().getPropName());
		Assert.assertEquals("trainerattributes",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(6).getLastPathComponent()).getColProp()
						.getType().getPropName());
	...
	public void testCreateTrainerPlanning() {
		...
		Assert.assertEquals("Bl" + Umlaut.L_UUML + "mel_Ulrike_",
				date.getTrainerplannings().iterator().next().getDefaulttrainers().iterator().next().getIdString());
		...
		Trainer martin = (Trainer) doc.findBean("org.rapidbeans.clubadmin.domain.Trainer",
				"Bl" + Umlaut.L_UUML + "mel_Martin");
	...
	public void testCreateLocationOk() {
		...
		DocumentTreeViewSwing masterTreeView = (DocumentTreeViewSwing) masterDocView.getTreeView();
		JTree masterTree = (JTree) masterTreeView.getTree();
		Assert.assertEquals("locations",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(treeViewIndexLocations).getLastPathComponent())
						.getColProp().getType().getPropName());
		...
		Assert.assertEquals("B",
				((RapidBean) masterTree.getPathForRow(treeViewIndex2).getLastPathComponent()).getIdString());
		Assert.assertEquals("Eurythmiesaal 1 Waldorfschule",
				((RapidBean) masterTree.getPathForRow(treeViewIndex3).getLastPathComponent()).getIdString());
	}
	...
	public void testCreateSalarySingle() throws InterruptedException {
		...
		Assert.assertEquals("trainerroles",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(3).getLastPathComponent()).getColProp()
						.getType().getPropName());
		...
		Assert.assertEquals(Umlaut.U_UUML + "berpr" + Umlaut.L_UUML + "fen",
				((JButton) buttons.get("apply")).getText());
	...
	public void testCreateSalaryMultipleEditMoneyAfter() throws InterruptedException {
		...
		for (RapidBean bean : salaries) {
			switch (i++) {
			case 0:
				Assert.assertEquals("Trainer_Abteilungsleiter,Fach" + Umlaut.L_UUML + "bungsleiter",
						bean.getIdString());
				break;
			case 1:
		...
		retry(1, 300,
				new Class[] { ValidationInstanceAssocTwiceException.class, ArrayIndexOutOfBoundsException.class,
	...
	public void testCreateSalaryMultipleEditMoneyBefore() {
		...
		// simulate pressing the "O. K." button
		retry(1, 200, new Class[] { ArrayIndexOutOfBoundsException.class, BeanNotFoundException.class,
				NullPointerException.class, ValidationInstanceAssocTwiceException.class, BeanDuplicateException.class },
				new RetryableAction() {
					public void doSomething() {
						editor.handleActionOk();
					}
	...
	public void testCreateSalaryAfterDuplicateWithAttributes() {
		...
		// simulate pressing the "OK" button
		retry(1, 500,
				new Class[] { ArrayIndexOutOfBoundsException.class, BeanDuplicateException.class,
		...
		for (RapidBean bean : salaries2) {
			...
			case 0:
				...
				break;
			case 1:
				Assert.assertEquals("Trainer_Abteilungsleiter,Fach" + Umlaut.L_UUML + "bungsleiter,Trainer A,Trainer B",
						bean.getIdString());
				break;
			case 2:
				Assert.assertEquals("Trainer_Fach" + Umlaut.L_UUML + "bungsleiter", bean.getIdString());
	...
	public void testCreateSalaryPersonalApplyConsecutive() {
		...
		PersonalSalary ps = (PersonalSalary) doc.findBean(PersonalSalary.class.getName(), "Trainer_Dahlheimer_Berit_");
		Assert.assertNull(ps);
		ps = (PersonalSalary) doc.findBean(PersonalSalary.class.getName(),
				"Trainer_Bl" + Umlaut.L_UUML + "mel_Martin_");
		Assert.assertNull(ps);
	...
	/**
	 * create a closing period add the to date via editor and edit a second time.
	...
	/**
	 * create a closing period and a location while selecting all closing periods
	...
	public void testCreateClosingPeriodAndLocation() throws InterruptedException {
		...
		JTree tree = (JTree) treeView.getTree();
		Assert.assertEquals("locations",
				((DocumentTreeNodePropColComp) tree.getPathForRow(treeViewIndex0).getLastPathComponent()).getColProp()
						.getType().getPropName());
		Assert.assertEquals("closingperiods",
				((DocumentTreeNodePropColComp) tree.getPathForRow(treeViewIndex1).getLastPathComponent()).getColProp()
						.getType().getPropName());
	 ...
	 * create a closing period and determine the location in one step.
	 * 
	 * @param editorCreateApplyMode editor create apply mode
	 * @throws InterruptedException
	 */
	private void testCreateClosingPeriodWithLocation(final CreateNewBeansEditorApplyBehaviour editorCreateApplyMode)
		...
		RapidBean cp1 = master.findBean("org.rapidbeans.clubadmin.domain.Location",
				"Turnhalle Grundschule S" + Umlaut.L_UUML + "d");
		...
		try {
			...
			final ClosingPeriod cp20010101 = (ClosingPeriod) master
					.findBean("org.rapidbeans.clubadmin.domain.ClosingPeriod", "20010101_Test");
			Assert.assertNotNull(cp20010101);
			Assert.assertEquals("Test", cp20010101.getName());
			Assert.assertEquals("20010101", cp20010101.getProperty("from").toString());
			retry(new RetryableAction() {
				public void doSomething() {
					...
					Assert.assertEquals("Turnhalle Grundschule S" + Umlaut.L_UUML + "d",
							locs.iterator().next().getIdString());
	...
	/**
	 * open the dialog to create a new closing period. insert a to date less than
	 * from and verify that the Apply button has the check text.
	 */
	@Test
	...
	/**
	 * Test deleting a training date which leads to a fairly complex delete cascade.
	 */
	@Test
	...
	public void testDeleteClosingPeriod() throws MalformedURLException {
		...
		File testfile = new File("src/test/resources/testDeleteClosingPeriod.xml");
	...
	/**
	 * Test if a bean editor is correctly updated. Change the collection property of
	 * one of two beans that are inversely linked with each other (ClosingPeriod and
	 * Location) and test the update of all property editors.
	 */
	@Test
	public void testEditorUpdateClosingPeriodLocation() {
		...
		Assert.assertEquals("locations",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(treeViewIndexLocations).getLastPathComponent())
						.getColProp().getType().getPropName());
		Assert.assertEquals("closingperiods",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(treeViewIndex2).getLastPathComponent())
						.getColProp().getType().getPropName());

		// check Location "Turnhalle Grundschule S" + Umlaut.L_UUML + "d"
		masterTree.setSelectionPath(masterTree.getPathForRow(treeViewIndex1));
		masterTreeView.editBeans();
		EditorBeanSwing edLoc = (EditorBeanSwing) masterDocView
				.getEditor((RapidBean) masterTree.getPathForRow(treeViewIndex1).getLastPathComponent(), false);
		Location loc = (Location) edLoc.getBean();
		Assert.assertEquals("Turnhalle Grundschule S" + Umlaut.L_UUML + "d",
				edLoc.getPropEditors().get(0).getInputFieldValue());
		...
		Assert.assertEquals("Eurythmiesaal 1 Waldorfschule",
				((RapidBean) propedCpLocs.getWidgetList().getModel().getElementAt(0)).getIdString());
	...
	public void testEditTrainer() {
		...
		DocumentViewSwing masterDocView = this.getTestviewMasterdata();
		Document master = client.getDocument("masterdata");
		Trainer martin = (Trainer) master.findBean("org.rapidbeans.clubadmin.domain.Trainer",
				"Bl" + Umlaut.L_UUML + "mel_Martin_");
		...
		ed.handleActionOk();
		Assert.assertEquals("martin.bluemel@gmx.de", ((Trainer) master
				.findBean("org.rapidbeans.clubadmin.domain.Trainer", "Bl" + Umlaut.L_UUML + "mel_Martin_")).getEmail());
	}
	 ...
	 * "Weihnachtsferien 20051222" - remove location "Eurythmiesaal Waldorfschule" -
	 * press O. K.
	 */
	@Test
	public void testEditClosingPeriodLocationDeleteSimple() {
		...
		Assert.assertEquals("closingperiods",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(cprow).getLastPathComponent()).getColProp()
						.getType().getPropName());
		...
		masterTreeView.editBeans();
		EditorBeanSwing ed = (EditorBeanSwing) masterDocView
				.getEditor((RapidBean) masterTree.getPathForRow(cprow + 1).getLastPathComponent(), false);
	...
	public void testEditClosingPeriodLocationDeleteApply() {
		...
		Assert.assertEquals(2, ped.getWidgetList().getModel().getSize());
		Assert.assertEquals("Eurythmiesaal 1 Waldorfschule",
				((RapidBean) ped.getWidgetList().getModel().getElementAt(0)).getIdString());
		Assert.assertEquals("Turnhalle Grundschule S" + Umlaut.L_UUML + "d",
	...
	public void testEditClosingPeriodLocationDeleteCancel() {
		...
		masterTree.setSelectionPath(masterTree.getPathForRow(cprow + 1));
		masterTreeView.editBeans();
		final EditorBeanSwing edCp = (EditorBeanSwing) masterDocView
				.getEditor((RapidBean) masterTree.getPathForRow(cprow + 1).getLastPathComponent(), false);
	...
	public void testEditClosingPeriodLocationMultipleLinks() {
		...
		masterTree.setSelectionPath(masterTree.getPathForRow(cprow + 1));
		masterTreeView.editBeans();
		final EditorBeanSwing ed = (EditorBeanSwing) masterDocView
				.getEditor((RapidBean) masterTree.getPathForRow(cprow + 1).getLastPathComponent(), false);
		Assert.assertSame(cp, ed.getBean());
	...
	/**
	 * Add and remove some links between a location and a closing period and check
	 * if the tree view is updated correctly.
	 */
	@Test
	public void testEditLocationClosingPeriodsAddRemoveWithTreeView() {
		...
		Assert.assertEquals("20070330_Osterferien",
				((ReadonlyListCollection<ClosingPeriod>) locEsaal1.getClosedons()).get(4).getIdString());
		...
		Assert.assertEquals("20051222_Weihnachtsferien", ((DocumentTreeNodeBeanLink) masterTree
				.getPathForRow(treeViewIndexLocationEsaal1 + 2).getLastPathComponent()).getLinkedBean().getIdString());
		Assert.assertEquals("20060116_Schulputztag", ((DocumentTreeNodeBeanLink) masterTree
				.getPathForRow(treeViewIndexLocationEsaal1 + 3).getLastPathComponent()).getLinkedBean().getIdString());
		Assert.assertEquals("20061223_Weihnachtsferien", ((DocumentTreeNodeBeanLink) masterTree
				.getPathForRow(treeViewIndexLocationEsaal1 + 4).getLastPathComponent()).getLinkedBean().getIdString());
		Assert.assertEquals("20070301_20-Jahre-Feier", ((DocumentTreeNodeBeanLink) masterTree
				.getPathForRow(treeViewIndexLocationEsaal1 + 5).getLastPathComponent()).getLinkedBean().getIdString());
		Assert.assertEquals("20070330_Osterferien", ((DocumentTreeNodeBeanLink) masterTree
				.getPathForRow(treeViewIndexLocationEsaal1 + 6).getLastPathComponent()).getLinkedBean().getIdString());

		int treeViewIndexLocationTurnhalle1 = treeViewIndexLocationEsaal1 + 7;
		Assert.assertSame(locTurnhalle,
				masterTree.getPathForRow(treeViewIndexLocationTurnhalle1).getLastPathComponent());
		Assert.assertSame(locTurnhalle.getProperty("closedons"), ((DocumentTreeNodePropCol) masterTree
				.getPathForRow(treeViewIndexLocationTurnhalle1 + 1).getLastPathComponent()).getColProp());
		Assert.assertEquals("20061223_Weihnachtsferien",
		...
		Assert.assertSame(cpXmas05.getProperty("locations"), ((DocumentTreeNodePropCol) masterTree
				.getPathForRow(treeViewIndexClosingperiodXmas05 + 1).getLastPathComponent()).getColProp());
		Assert.assertSame(locEsaal1, ((DocumentTreeNodeBeanLink) masterTree
				.getPathForRow(treeViewIndexClosingperiodXmas05 + 2).getLastPathComponent()).getLinkedBean());
		...
		retry(new Class[] { ClassCastException.class, NullPointerException.class }, new RetryableAction() {
			public void doSomething() {
				Assert.assertSame(locEsaal1.getProperty("closedons"), ((DocumentTreeNodePropCol) masterTree
						.getPathForRow(treeViewIndexLocationEsaal1 + 1).getLastPathComponent()).getColProp());
				Assert.assertEquals("20060116_Schulputztag",
		...
		retry(new Class[] { ClassCastException.class, NullPointerException.class }, new RetryableAction() {
			public void doSomething() {
				...
				Assert.assertSame(locTurnhalle,
						masterTree.getPathForRow(treeViewIndexLocationTurnhalle3).getLastPathComponent());
				Assert.assertSame(locTurnhalle.getProperty("closedons"), ((DocumentTreeNodePropCol) masterTree
						.getPathForRow(treeViewIndexLocationTurnhalle3 + 1).getLastPathComponent()).getColProp());
				Assert.assertEquals("20051222_Weihnachtsferien",
						((DocumentTreeNodeBeanLink) masterTree.getPathForRow(treeViewIndexLocationTurnhalle3 + 2)
								.getLastPathComponent()).getLinkedBean().getIdString());
				Assert.assertEquals("20061223_Weihnachtsferien",
						((DocumentTreeNodeBeanLink) masterTree.getPathForRow(treeViewIndexLocationTurnhalle3 + 3)
								.getLastPathComponent()).getLinkedBean().getIdString());
				Assert.assertEquals("20070330_Osterferien",
						((DocumentTreeNodeBeanLink) masterTree.getPathForRow(treeViewIndexLocationTurnhalle3 + 4)
								.getLastPathComponent()).getLinkedBean().getIdString());

				Assert.assertSame(cpXmas05,
						masterTree.getPathForRow(treeViewIndexClosingperiodXmas05).getLastPathComponent());
				Assert.assertSame(cpXmas05.getProperty("locations"), ((DocumentTreeNodePropCol) masterTree
						.getPathForRow(treeViewIndexClosingperiodXmas05 + 1).getLastPathComponent()).getColProp());
				Assert.assertSame(locTurnhalle, ((DocumentTreeNodeBeanLink) masterTree
						.getPathForRow(treeViewIndexClosingperiodXmas05 + 2).getLastPathComponent()).getLinkedBean());
			}
		});
	}
	...
	public void testEditClosingPeriodOnedayToMoreDays() throws InterruptedException {
		...
		masterTreeView.editBeans();
		EditorBeanSwing ed = (EditorBeanSwing) masterDocView
				.getEditor((RapidBean) masterTree.getPathForRow(cprow + 2).getLastPathComponent(), false);
	...
	/**
	 * Tests deleting a location while editing a ClosingPeriod. - open ClosingPeriod
	 * "Weihnachtsferien 20051222" - remove 1st Location "Eurythmiesaal 1
	 * Waldorfschule" - press OK
	 */
	@Test
	public void testEditClosingPeriodLocationBeanRemoved() {
		...
		masterTree.setSelectionPath(masterTree.getPathForRow(cprow + 1));
		masterTreeView.editBeans();
		ed = (EditorBeanSwing) masterDocView
				.getEditor((RapidBean) masterTree.getPathForRow(cprow + 1).getLastPathComponent(), false);
		Assert.assertEquals("Weihnachtsferien", ed.getPropEditors().get(0).getInputFieldValue());
		ped = (EditorPropertyListSwing) ed.getPropEditor("locations");
		Assert.assertEquals(1, ped.getWidgetList().getModel().getSize());
		loc = (Location) ped.getProperty().getBean().getContainer().findBean("org.rapidbeans.clubadmin.domain.Location",
				"Eurythmiesaal 1 Waldorfschule");
	...
	/**
	 * Tests a sequence that formerly lead to a concurrent modification exception. -
	 * open location "Eurythmiesaal 1 Waldorfschule" in a first editor - open
	 * closing period "Weihnachtsferien 20051222" in a second editor - unselect
	 * checkbox "Eurythmiesaal 1 Waldorfschule" - select checkbox "Eurythmiesaal 1
	 * Waldorfschule" again changed from Checkboxes to List so: - remove
	 * "Eurythmiesaal 1 Waldorfschule" - add "Eurythmiesaal 1 Waldorfschule" again
	 */
	@Test
	public void testEditLocationAndClosingPeriodCloseCp() {
		...
		Assert.assertEquals("locations",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(locationrow).getLastPathComponent())
						.getColProp().getType().getPropName());
		Assert.assertEquals("closingperiods",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(locationrow + 1).getLastPathComponent())
						.getColProp().getType().getPropName());
		...
		Assert.assertEquals("locations",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(locationrow).getLastPathComponent())
						.getColProp().getType().getPropName());
		Assert.assertEquals("closingperiods",
				((DocumentTreeNodePropColComp) masterTree.getPathForRow(locationrow + 3).getLastPathComponent())
						.getColProp().getType().getPropName());
		...
		masterTreeView.editBeans();
		EditorBeanSwing ed1 = (EditorBeanSwing) masterDocView
				.getEditor((RapidBean) masterTree.getPathForRow(locationrow + 1).getLastPathComponent(), false);
		...
		masterTreeView.editBeans();
		EditorBeanSwing ed2 = (EditorBeanSwing) masterDocView
				.getEditor((RapidBean) masterTree.getPathForRow(locationrow + 4).getLastPathComponent(), false);
		...
		Assert.assertEquals("Eurythmiesaal 1 Waldorfschule",
				((RapidBean) proped2.getWidgetList().getModel().getElementAt(0)).getIdString());
		Assert.assertEquals("Turnhalle Grundschule S" + Umlaut.L_UUML + "d",
				((RapidBean) proped2.getWidgetList().getModel().getElementAt(1)).getIdString());
	...
	/**
	 * First test with the normal "trainingslist" view. A training held by trainer
	 * with null role must not be accepted.
	 */
	@Test
	...
	// Assert.assertEquals("Stadtsparkasse M�nchen", ((CreditInstitute)
	...
	// //assertEquals("Hypovereinsbank M�nchen", ((CreditInstitute)
	// //masterTree.getPathForRow(10).getLastPathComponent()).getName());
	// //assertEquals("Stadtsparkasse M�nchen", ((CreditInstitute)
	...
	public void testTreeViewShowTrainings() {
		...
		Assert.assertEquals("trainings",
				((DocumentTreeNodePropColComp) tree.getPathForRow(clubsrow + 8).getLastPathComponent()).getColProp()
						.getType().getPropName());
	...
	public void testTrainingSetTrainerTwice() {
		...
		try {
			...
			Assert.assertTrue(
					e.getMessage().startsWith("invalid value \"Dahlheimer_Berit_\" for property \"trainer\"."));
		}
	}
	...
	private Document getTestdocMasterdata() {
		if (this.docMaster == null) {
			client.openDocumentView(new Document(new File("src/test/resources/masterdata.xml")), "masterdata",
					"standard");
	...
	private DocumentViewSwing getTestviewMasterdata() {
		retry(new Class[] { RapidBeansRuntimeException.class, ValidationException.class, }, new RetryableAction() {
			...
			public void doSomething() {
				if (docMaster == null) {
					client.openDocumentView(new Document("masterdata", new File("src/test/resources/masterdata.xml")),
							"masterdata", "standard");
	...
	private DocumentViewSwing getTestviewTrainingsListExpert() {
		if (this.docTrainingsList == null) {
			final File testfile = new File("src/test/resources/trainingslist_20060101_20060331.xml");
	...
	private static class ApplicationMock extends RapidClubAdminClient {
		...
		public void start() {
			...
			this.setConfiguration(
					(ConfigApplication) new Document(TypeRapidBean.forName(ConfigApplication.class.getName()),
							new File("src/main/resources/org/rapidbeans/clubadmin/Application.xml")).getRoot());
			this.getMasterDoc();
		...
		public Document getMasterDoc() {
			if (this.masterDoc == null) {
				try {
					...
					this.masterDoc = new Document(new File("src/test/resources/masterdata.xml"));
				} finally {
