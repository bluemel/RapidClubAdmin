...
public final class DbExporterVerdat {
	...
	private static final Logger log = Logger.getLogger(DbExporterVerdat.class.getName());
	...
	public void export() {
		if (this.toFile == null) {
			throw new RapidClubAdminBusinessLogicException("export.failure.exportfilenotdefined",
					"Target file for export is not defined.");
		}
		if (!this.toFile.exists()) {
			throw new RapidClubAdminBusinessLogicException("export.failure.exportfilenotexistent",
					"Target file for export does not exist.");
		}
		...
		if (exportJob.getBillingPeriod().getDateExportFirst() == null) {
			...
			exportJob.getBillingPeriod().setDateExportFirst(exportJob.getBillingPeriod().getDateClosing());
		}
		try {
			...
			for (Trainer trainer : exportJob.getTrainers()) {
				...
				for (ExportJobEntry entry : exportJob.getExportEntries(trainer)) {
					log.fine("inserting entry: " + entry.getTrainer().getIdString() + ", "
	...
	/**
	 * An Uebungleiter is externally identified 1) by it's external system ID if
	 * this is set 2) by "<firstname> <lastname>" otherwise
	 *
	 ...
	 */
	private String insertUebungsleiterIfNotInserted(final Trainer trainer) throws SQLException, InterruptedException {
		String uelId = this.findUelId(trainer);
		if (uelId != null) {
			log.fine("Übungsleiter for Trainer \"" + trainer.getIdString() + "\" already exists.");
			return uelId;
		}

		log.fine("Inserting Übungleiter for Trainer \"" + trainer.getIdString() + "\"...");
		final Statement statement = con.createStatement();
		String trainername = trainer.getLastname();
		if (trainername == null) {
			throw new RuntimeException("ERROR during inserting trainer \"" + trainer.getIdString() + " to MS Acces:\n"
					+ "Trainer has no \"last name\" specified.");
		}
		if (trainer.getFirstname() != null) {
			trainername = trainer.getFirstname() + " " + trainername;
		}
		String sql = "INSERT INTO Übungsleiter" + " (UName";
		...
		if (statement.executeUpdate(sql) != 1) {
			throw new RuntimeException("ERROR during insert trainer \"" + trainer.getIdString() + " to Übungsleiter");
		}
		...
		ExportJobResultEntry resultEntry = new ExportJobResultEntry(ExportJobResultEntryModificationType.create,
				"Übungsleiter", uelId);
	...
	private String insertUelAbrechnungIfNotInserted(final Trainer trainer, final String uelId,
			final ExportJobEntry entry) throws SQLException, InterruptedException {
		String uelAbrechnungId = this.findUelAbrechnungId(uelId);
		if (uelAbrechnungId != null) {
			log.fine("Übungleiter Abrechnung for Trainer \"" + trainer.getIdString() + "\" on date \""
					+ exportJob.getDateFirstExportString() + "\" already exists.");
			return uelAbrechnungId;
		}
		log.fine("Inserting Übungleiter Abrechnung for Trainer \"" + trainer.getIdString() + "\" on date \""
				+ exportJob.getDateFirstExportString() + "\"...");
		final Statement statement = con.createStatement();
		String sql = "INSERT INTO ÜbungsleiterAbrechnung" + " (UID, UARechnungID, UARechnungNR, UADatum, Status"
				+ ", KostenstelleID, FIBUKONTO, FIBUGKONTO, DM, WährungKZ"
				+ ", MwstID, Mwst, MwstBetrag, GBetrag, Gebucht, DTAText" + ") VALUES (" + uelId + ", "
				+ getAbrechnungsId(uelId) + ", '" + getAbrechnungsNummer(uelId) + "'" + ", " + getAbrechnungsDatum()
				+ ", " + abrechnungsStatus + ", '" + abrechnungsKostenstellenID + "'" + ", " + abrechnungsFibukonto
				+ ", " + abrechnungsFibuGKonto + ", '" + abrechnungsWaehrung + "'" + ", "
				+ abrechnungsWaehrungsKennziffer + ", " + abrechnungsMwstID + ", " + abrechnungsMwst + ", "
				+ abrechnungsMwstBetrag + ", " + getAbrechnungsGBetrag(trainer) + ", " + abrechnungsGebucht + ", '"
				+ this.getAbrechnungsDtaText() + "'" + ");";
		log.fine("update SQL: " + sql);
		if (statement.executeUpdate(sql) != 1) {
			throw new RuntimeException("ERROR during Inserting" + " ÜbungsleiterAbrechnung for Trainer \""
					+ trainer.getIdString() + "\" and date \"" + exportJob.getDateFirstExportString() + "\"");
		}
	...
	private String insertUelBerichIfNotInserted(final Trainer trainer, final String uelId, final ExportJobEntry entry)
		...
		if (uelBereichId != null) {
			log.fine("ÜbungleiterBereich for Trainer \"" + trainer.getIdString() + "\" and for SalaryComponentType \""
					+ bezeichnung + "\" already exists.");
			return uelBereichId;
		}

		log.fine("Inserting ÜbungleiterBereich for Trainer \"" + trainer.getIdString()
				+ "\" and for SalaryComponentType \"" + bezeichnung + "\"...");
		final Statement statement = con.createStatement();
		String sql = "INSERT INTO ÜbungsleiterBereich" + " (UID, Bezeichnung, Einheit, Preis) VALUES (" + uelId + ", '"
				+ bezeichnung + "'" + ", '" + getUelStundenEinheit(entry) + "'" + ", " + getUelStundenPreis(entry)
				+ ");";
		log.fine("update SQL: " + sql);
		if (statement.executeUpdate(sql) != 1) {
			throw new RuntimeException("ERROR during Inserting" + " ÜbungsleiterBereich for Trainer \""
					+ trainer.getIdString() + "\" and for SalaryComponentType \"" + bezeichnung + "\"");
		}
	...
	private String insertUelStundenIfNotInserted(final Trainer trainer, final String uaId, final String ubId,
			final ExportJobEntry entry) throws SQLException, InterruptedException {
		String uelStundenId = this.findUelStundenId(uaId, ubId, this.getUelBereichBezeichnung(entry));
		if (uelStundenId != null) {
			log.fine("ÜbungsleiterStunden for Abrechnung \"" + uaId + "\" and Uebungsleiter \"" + ubId
					+ "\" already exists.");
			return uelStundenId;
		}

		log.fine("Inserting ÜbungsleiterStunden for Trainer \"" + trainer.getIdString() + "\" with name \""
		...
		final String sql = "INSERT INTO �bungsleiterStunden" + " (UAID, UBID, USDatum, USMenge, USPreis"
				+ ", USBetrag, USBezeichnung, USEinheit) VALUES (" + uaId + ", " + ubId + ", " + "'"
				+ df.format(exportJob.getDateFirstExport()) + "'" + ", " + menge + ", " + preis + ", " + betrag + ", '"
				+ bezeichnung + "'" + ", '" + getUelStundenEinheit(entry) + "');";
		log.fine("update SQL: " + sql);
		if (statement.executeUpdate(sql) != 1) {
			throw new RuntimeException("ERROR during Inserting" + " ÜbungsleiterStunden for Trainer \""
					+ trainer.getIdString() + "\" with name \"" + bezeichnung + "\"");
		}
	...
	private String findUelId(final Trainer trainer) throws SQLException {
		String uelId = null;
		final Statement statement = con.createStatement();
		final String query = "SELECT UId FROM �bungsleiter WHERE UName = '" + trainer.getFirstname() + " "
				+ trainer.getLastname() + "';";
		...
		if (!resultSet.next()) {
			log.fine("Übungsleiter Id of Trainer \"" + trainer.getIdString() + "\": not found");
		} else {
			uelId = resultSet.getString(1);
			log.fine("Found �bungsleiter Id from Trainer \"" + trainer.getIdString() + "\": " + uelId);
		}
		return uelId;
	}
	...
	private String findUelAbrechnungId(final String uelId) {
		...
		try {
			final Statement statement = con.createStatement();
			String query = "SELECT UAID FROM �bungsleiterAbrechnung" + " WHERE UID = " + uelId + " AND UADatum = "
					+ this.getAbrechnungsDatum() + ";";
			log.fine("query = \"" + query + "\"");
			if (!statement.execute(query)) {
				throw new RuntimeException("ERROR during select Uid from �bungsleiter");
			}
			final ResultSet resultSet = statement.getResultSet();
			if (!resultSet.next()) {
				log.fine("ÜbungsleiterAbrechnung Id for Uebungleiter \"" + uelId + "\" with date = "
	...
	/**
	 * Find an ÜbungsleiterBereich identified uniquely by the Übungsleiter's ID and
	 * its Bezeichnung.
	 * 
	 * @param entry the ExportJobEntry defining the earned money
	 * @param uelId the Übungsleiter ID
	 *
	 * @return the String define�ng the unique ID of an ÜbungsleiterBereich or null
	 *         if not found.
	 */
	private String findUelBereichId(final ExportJobEntry entry, final String uelId) {
		...
		try {
			...
			String query = "SELECT UBID FROM ÜbungsleiterBereich" + " WHERE UID = " + uelId + " AND Bezeichnung = '"
					+ bezeichnung + "';";
			log.fine("query = \"" + query + "\"");
			if (!statement.execute(query)) {
			...
			final ResultSet resultSet = statement.getResultSet();
			if (!resultSet.next()) {
				log.fine("Übungsleiter Bereich for Uebungleiter \"" + uelId + "\" and with Bezeichnung \"" + bezeichnung
						+ "\": not found");
			} else {
	...
	private String findUelStundenId(final String uaId, final String ubId, final String bez) {
		String uelStundenId = null;
		try {
			final Statement statement = con.createStatement();
			String query = "SELECT USID FROM ÜbungsleiterStunden" + " WHERE UAID = " + uaId + " AND UBID = " + ubId
			...
			final ResultSet resultSet = statement.getResultSet();
			if (!resultSet.next()) {
				log.fine("Übungsleiter Stunden for UebungleiterAbrechnung \"" + uaId
						+ "\" and for ÜbungsleiterBereich \"" + ubId + "\" and for Bezeichnung: \"" + bez
						+ "\" not found");
			} else {
	...
	private String getAbrechnungsDtaText() {
		return "Honorar " + toMonthYearNrString(this.exportJob.getBillingPeriod().getFrom()) + " - "
...
//        return entry.getSalaryComponentType().getName();
	...
	private String getUelStundenEinheit(final ExportJobEntry entry) {
		...
		case 60:
			...
			break;
		default:
			einheit = Integer.toString(minutes) + " " + "Min.";
			break;
		}
		return einheit;
	}
	...
	private String getUelStundenPreis(final ExportJobEntry entry) {
		return nf
				.format(entry.getSalary().getComponent(entry.getSalaryComponentType()).getMoney().getMagnitudeDouble());
	}
...
//				+ this.toFile.getPath() + ";DriverID=22;READONLY=true}", "", "");
		return DriverManager.getConnection("jdbc:ucanaccess://" + this.toFile.getPath() + ";memory=true", "", "");
	}
	...
	public static String reportResult(final ExportJob exportJob) {
		...
		sb.append(PlatformHelper.getLineFeed());
		int i = 1;
		for (ExportJobResultEntry entry : exportJob.getResultEntries(ExportJobResultEntryModificationType.create,
				"Übungsleiter")) {
			sb.append("    ");
	...
	private static void appendAttrVal(final StringBuffer sb, final ExportJobResultEntry entry, final String attrname) {
		sb.append(", ");
		sb.append(attrname);
		sb.append(": ");
