...
public class TrainerPlanningPropDefaulttrainers extends PropertyCollection {
	...
	/**
	 * Validate the trainer to be assigned as default trainer for this trainer
	 * planning.<br>
	 *
				...
				if (ldefaulttrainer != null && ldefaulttrainer instanceof RapidBean) {
					...
					if ((defaulttrainer.getDepartments() == null)
						...
						throw new ValidationException(
								"invalid.prop.trainerplanning.defaulttrainer.notauthorized.department", this.getBean(),
								"invalid value \"" + ldefaulttrainer.getIdString()
										+ "\" for property \"defaultrainer\".\n"
										+ "This trainer is not authorized for department (" + department.getIdString()
										+ ").",
								args1);
					}
					// - same default trainer within same parent TrainigDate must be used only once.
					// - default trainer must not be planned within another training date with
					// overlapping time.
					final Collection<?> plannings = defaulttrainer.getTrainerplannings();
					if (plannings != null) {
						...
						for (Object oTrplan : plannings) {
							...
							if (!(oTrplan instanceof TrainerPlanning)) {
								throw new RapidClubAdminBusinessLogicException("xxx",
										"unexpected class \"" + oTrplan.getClass().getName() + "\"");
							}
									...
									if (trplanDate.getDayofweek() == parentDate.getDayofweek()) {
										if (((trplanDate.getTimestart().compareTo(parentDate.getTimestart()) > 0)
												...
												|| (trplanDate.getTimeend().compareTo(parentDate.getTimestart()) > 0)
														&& (trplanDate.getTimeend()
																.compareTo(parentDate.getTimeend()) < 0)) {
											final Object[] args3 = { ldefaulttrainer, trplanDate,
													trplanDate.getLocation() };
											throw new ValidationException(
													...
													"invalid value \"" + ldefaulttrainer.getIdString()
															...
															+ trplanDate.getLocation().getIdString() + "\"",
													args3);
										}
									}
								}
