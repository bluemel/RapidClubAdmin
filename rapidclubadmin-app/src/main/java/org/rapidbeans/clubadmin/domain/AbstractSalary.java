...
public abstract class AbstractSalary extends RapidBeanBaseAbstractSalary {
	...
	/**
	 * initialize the salary's time according to the MasterData's trainerhour if
	 * accessible.
	 */
	protected void init() {
	...
	/**
	 * The key property "rolename" is hereby bound to the name of the parent bean.
	...
	/**
	 * compare two salaries. normalize to money / hour.
	 *
			...
			if (comps != null && comps.size() > 0) {
				...
				for (SalaryComponent comp : comps) {
					if (currency == null) {
						...
						if ((comp.getMoney() != null) && (!(currency == comp.getMoney().getUnit()))) {
							throw new RapidClubAdminBusinessLogicException("salary.component.money.unit.diff",
									"Different salary component currency \"" + comp.getMoney().getUnit().toString()
											+ "\"");
						}
					}
					if (time == null) {
						...
						if (!(time.equals(comp.getTime()))) {
							throw new RapidClubAdminBusinessLogicException("salary.component.time.diff",
									"Different salary component time \"" + comp.getTime().toString() + "\"");
						}
					}
				...
				if (this.getMoney() == null || (!(this.getMoney().getMagnitude().equals(sum)))) {
