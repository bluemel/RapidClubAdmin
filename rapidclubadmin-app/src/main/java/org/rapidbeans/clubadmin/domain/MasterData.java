	...
	public void generateUsersForTrainers() {
		...
		for (final Trainer trainer : this.getTrainers()) {
			if (trainer.getUser() == null) {
				...
				if (this.getContainer().findBean(ClubadminUser.class.getName(), accountname) != null) {
					accountname = trainer.getLastname().toLowerCase() + "_"
							+ trainer.getFirstname().toLowerCase().substring(0, 1);
					if (this.getContainer().findBean(ClubadminUser.class.getName(), accountname) != null) {
						accountname = trainer.getLastname().toLowerCase() + "_" + trainer.getFirstname().toLowerCase();
						if (this.getContainer().findBean(ClubadminUser.class.getName(), accountname) != null) {
							accountname = trainer.getLastname().toLowerCase() + "_"
							...
							;
							int i = 0;
							while (this.getContainer().findBean(ClubadminUser.class.getName(), accountname) != null) {
								accountname = trainer.getLastname().toLowerCase() + "_"
										+ trainer.getFirstname().toLowerCase() + "_"
										+ trainer.getMiddlename().toLowerCase().substring(0, 1) + Integer.toString(i);
								i++;
							}
						}
					}
