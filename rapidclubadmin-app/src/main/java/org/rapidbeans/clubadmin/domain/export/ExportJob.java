...
/**
 * DB export business logic. List of entries to be exported to the DB.
 *
 * @author Martin Bluemel
 */
public final class ExportJob {

	private static final Logger log = Logger.getLogger(ExportJob.class.getName());
	...
	public void addResultEntry(final ExportJobResultEntry entry) {
		...
		if (this.result.get(key) != null) {
			throw new RapidBeansRuntimeException("Export entry \"" + key + "\" is already added");
		}
	...
	public List<ExportJobResultEntry> getResultEntries(final ExportJobResultEntryModificationType modificationType,
		...
		for (ExportJobResultEntry entry : this.result.values()) {
			if (entry.getModificationType() == modificationType && entry.getEntityType().equals(entityType)) {
	 ...
	 */
	private void addExportEntries(final Trainer trainer, final List<ExportJobEntry> entries) {
	...
	/**
	 * determine export job for a given trainer and a given set of departements.
	 *
	 * @param period      the BillingPeriod of this ExportJob
	 * @param departments the Departments
	 */
	public static ExportJob generateExportJob(final BillingPeriod period, final Department[] departments) {
	...
	/**
	 * Determine export entries for a given trainer and a given set of departments.
	 *
	 * @param departments the Departments
	 * @param trainer     the trainer
	 */
	final static List<ExportJobEntry> determineExportEntries(final Department[] departments, final Trainer trainer) {
	 ...
	 */
	private static List<ExportJobEntry> determineExportEntries(final Trainer trainer,
		...
		for (RapidBean bean : allTrhbts) {
			...
			if (sal != null && sal.getComponents() != null && sal.getComponents().size() > 0) {
				for (SalaryComponent sc : sal.getComponents()) {
					log.finer("sc[" + sc.getSalaryComponentType().getName() + "].getMoney = " + sc.getMoney());
					...
					final Money compMoneyEarned = new Money(new BigDecimal(moneyEarned.getMagnitudeDouble() * factor),
							(Currency) moneyEarned.getUnit());
					log.finer("money earned [" + sc.getSalaryComponentType().getName() + "]: " + compMoneyEarned);
		...
		List<Entry<SalaryComponentType, Money>> entryList = new ArrayList<Entry<SalaryComponentType, Money>>(size);
		...
		for (Entry<SalaryComponentType, Money> entry : entryList) {
			log.finer("trainer: " + trainer.getIdString() + ", sctype: " + entry.getKey().getName() + ", worked: "
					+ sumTrainerHours + ", earned: " + entry.getValue());
	...
	final public static void sort(final List<TrainingHeldByTrainer> trhbts) {
		...
		for (int i = 0; i < size; i++) {
			for (int j = i + 1; j < size; j++) {
				if (isGreater(trhbts.get(j), (TrainingHeldByTrainer) trhbts.get(i))) {
