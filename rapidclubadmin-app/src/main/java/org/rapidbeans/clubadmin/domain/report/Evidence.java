...
public final class Evidence {
	 ...
	 * @param singleSheets if true single formulars will be printed, if false
	 *                     coherent sheets will be printed
	 */
	...
	final static void printReportSheets(final FileWriter out, final Template template, final Department[] departments,
		...
		final Trainer trainer = (Trainer) departments[0].getContainer()
				.findBeanByQuery("org.rapidbeans.clubadmin.domain.Trainer[id = '" + trainerId + "']");
		printReportSheets(out, template, departments, trainer, singleSheets);
	}
	 ...
	 * @param pageCount    the count of pages to print for one single Trainer
	 *                     combined with one single department.
	 * @param singleSheets if true single formulars will be printed, if false
	 *                     coherent sheets will be printed
	...
	private static Time generateEvidenceBody(final FileWriter out, final String rtfTemplate, final Club club,
		...
		if ((!singleSheets) || pageCount == 1) {
			map.put("@BS@", PropertyDate.formatDate(trList.getFrom(), locale, DateFormat.MEDIUM, DateFormat.MONTH_FIELD)
					+ "."
					+ PropertyDate.formatDate(trList.getFrom(), locale, DateFormat.MEDIUM, DateFormat.YEAR_FIELD));
			...
			map.put("@BE@", PropertyDate
					.formatDate(((TrainingRegular) trhbts.get(trhbts.size() - 1).getParentBean()).getDate(), locale));
		}
		if ((!singleSheets) && (pageCount > 1)) {
		...
		for (TrainingHeldByTrainer trhbt : trhbts) {
			...
			try {
				trainerhoursWorked = new Time(
						hoursWorked.convert(UnitTime.min).getMagnitude().divide(trainerHour.getMagnitude()),
						UnitTime.min);
			...
			case checked:
				...
				map.put("@02" + lineNo + "@",
						training.getTimestart().toString() + " - " + training.getTimeend().toString());
		...
		try {
			sumTrainerhoursWorked = new Time(
					sumHours.convert(UnitTime.min).getMagnitude().divide(trainerHour.getMagnitude()), UnitTime.min);
		} catch (ArithmeticException e) {
	...
	/**
	 * find the report template file in the settings dir and copy it from res if it
	...
	public static File findEvidenceReportTemplateFile(final boolean keepLocalTemplateFile) {
		try {
			...
			File templateFile = new File(
					"res/org/rapidbeans/clubadmin/reports/" + locale.getName() + "/templateNachweis.rtf");
			...
			InputStream is = RapidClubAdmin.class
					.getResourceAsStream("reports/" + locale.getName() + "/templateNachweis.rtf");
			if (is == null) {
