...
public final class RapidClubAdmin {
	...
	public static void main(final String[] args) {
		final org.rapidbeans.clubadmin.presentation.RapidClubAdminClient appl = new org.rapidbeans.clubadmin.presentation.RapidClubAdminClient();
		...
		if (options.get("loaddocuments") != null) {
			final List<String> docstoloada = StringHelper.split((String) options.get("loaddocuments"),
					File.pathSeparator);
			for (String docfilename : docstoloada) {
				final File docfile = new File(docfilename);
				final Document doc = new Document(docfile);
				ApplicationManager.getApplication().openDocumentView(doc, "trainingslist", "trainings");
			}
		}
	}
}
