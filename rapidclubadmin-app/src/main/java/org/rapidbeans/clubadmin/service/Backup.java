...
public class Backup extends Action {
	...
	public void execute() {
		...
		case intermediate:
			...
			final String backupDirBpName = bp.getProperty("from").toString() + '_' + bp.getProperty("to").toString()
		...
		case finalmode:
			final String backupSubdir = "history/" + bp.getProperty("from").toString() + '_'
					+ bp.getProperty("to").toString();
			try {
				URL targetUrl = null;
				switch (app.getRunMode()) {
				case local:
					final String mfilename = app.getMasterDoc().getUrl().getFile()
							.replaceFirst("masterdata.xml", backupSubdir + "/masterdata.xml").replace("%20", " ");
					final File mfile = new File(mfilename);
					if (!mfile.getParentFile().exists()) {
						if (!mfile.getParentFile().mkdirs()) {
							throw new RapidBeansRuntimeException(
									"Problems to create folder \"" + mfile.getParent() + "\"");
						}
					}
				...
				for (Department dep : bp.getDepartments()) {
					...
					if (depTrlistDoc.getUrl().getProtocol().equals("file")) {
						...
						if (!file.getParentFile().exists()) {
							if (!file.getParentFile().mkdirs()) {
								throw new RapidBeansRuntimeException(
										"Problems to create folder \"" + file.getParent() + "\"");
							}
						}
					...
					final String dirs = StringHelper.splitBeforeLast(targetUrl.toString(), "/")
							.replaceFirst("^.*(history)", "$1");
