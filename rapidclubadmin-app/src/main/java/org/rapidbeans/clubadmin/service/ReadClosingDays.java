...
public class ReadClosingDays extends Action {
	...
	public void execute() {
		...
		if (currentMonth < 11) {
			foundClosingPeriods.addAll(
					new SchulferienReader().readSchulferienAndFeiertage("www.schulferien.org", "bayern", currentYear));
		}
		if (currentMonth > 6) {
			foundClosingPeriods.addAll(
					new SchulferienReader().readSchulferienAndFeiertage("www.schulferien.org", "bayern", nextYear));
		}
		...
		for (final ClosingPeriod cp : foundClosingPeriods) {
			if (!masterdata.getClosingperiods().contains(cp) && (!isContainedIn(masterdata.getClosingperiods(), cp)
					&& (!isContainedIn(newClosingPeriods, cp)))) {
