...
public class EditorClosingPeriod extends EditorBeanSwing {
	 ...
	 */
	public EditorClosingPeriod(final Application client, final DocumentView docView, final RapidBean bean,
	...
	public void inputFieldChanged(final EditorProperty propEditor) {
		...
		if (propEditor.getProperty().getName().equals("from") || propEditor.getProperty().getName().equals("to")) {
			...
			if (getPropEditor("oneday").getProperty().getValue().equals(Boolean.FALSE) && from != null && to != null
					&& from.equals(to)) {
				try {
