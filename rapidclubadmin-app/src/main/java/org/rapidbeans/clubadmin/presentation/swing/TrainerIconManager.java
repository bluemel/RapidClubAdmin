...
/**
 * Manages an caches Trainer icons. Mirrors icons between a (remote) repository
 * and a local folder.
 *
...
public class TrainerIconManager {
	 ...
	 * @param iconRepoType   LOCAL or REMOTE Defaults to REMOTE
	 * @param iconRepository the relative path the icon repository folder. Defaults
	 *                       to to "trainerIcons"
	 * @param iconListDoc    the relative path of the icon list file. Defaults to
	 *                       "trainerIcons/iconlist.xml" if null
	 * @param localIconCache the relative path to the local icon cache folder.
	 *                       Defaults to "&lt;settings dir&gt;/trainerIcons" if null
	 */
	public TrainerIconManager(final RapidClubAdminClient app, final int iconRepoType, final String iconRepository,
			final String iconListDoc, final String localIconCache) {
		if (app == null) {
			this.application = ((RapidClubAdminClient) ApplicationManager.getApplication());
		} else {
		...
		if (iconRepository != null) {
			...
			if (iconRepoType == ICON_REPOSITORY_TYPE_LOCAL) {
				...
				if (!f.exists()) {
					throw new RapidBeansRuntimeException("file not found \"" + f.getAbsolutePath() + "\"");
				}
				if (!f.isDirectory()) {
					throw new RapidBeansRuntimeException("file is not a folder \"" + f.getAbsolutePath() + "\"");
				}
			}
		}
		if (iconListDoc != null) {
			...
			if (iconRepoType == ICON_REPOSITORY_TYPE_LOCAL) {
				...
				if (!f.isFile()) {
					throw new RapidBeansRuntimeException("file is not a normal file \"" + f.getAbsolutePath() + "\"");
				}
			}
		}
		if (localIconCache != null) {
			...
			if (!this.localIconCache.exists()) {
				throw new RapidBeansRuntimeException(
						"file not found \"" + this.localIconCache.getAbsolutePath() + "\"");
			}
			if (!this.localIconCache.isDirectory()) {
				throw new RapidBeansRuntimeException(
						"file is not a folder \"" + this.localIconCache.getAbsolutePath() + "\"");
			}
		}
	}
	...
	static {
		try {
			defaultIcon = new ImageIcon(ImageIO.read(Application.class.getResource("pictures/team1.gif"))
		...
		if (trainer != null) {
			...
			if (iconFile.exists()) {
				try {
					...
					if (cachedIcon == null || cachedIcon.getLastModified() < iconFile.lastModified()) {
						trainerIcon = new ImageIcon(readScaledLocalFile(iconFile));
						final CachedIcon newCachedIcon = new CachedIcon(trainerIcon, iconFile.lastModified());
	...
	public void importIcon(final Trainer trainer, final File iconfile) {
		...
		if (this.application.getSettingsRapidClubAdmin().getShareiconsoverweb()) {
			...
			log.fine("import icon: loaded icon list document, found " + iconInfoList.getIcons().size() + " icons");
			iconInfoList.markIconUpdate(trainer);
			this.application.save(iconDoc);
			log.fine("import icon: wrote icon list document \"" + iconDoc.getUrl() + "\"");
		}
	}
	...
	public void deleteIcon(final Trainer trainer) {
		final File iconFile = new File(getLocalIconCache(), trainer.getIdString() + ".jpg");
		...
		if (this.application.getSettingsRapidClubAdmin().getShareiconsoverweb()) {
			...
			log.fine("delete icon: loaded icon list document, found " + iconInfoList.getIcons().size() + " icons");
			iconInfoList.removeIcon(iconInfoList.getIconInfo(trainer));
			this.application.save(iconDoc);
			log.fine("delete icon: wrote icon list document \"" + iconDoc.getUrl() + "\"");
			if (this.iconRepositoryType == ICON_REPOSITORY_TYPE_REMOTE) {
	 ...
	 */
	public void updateIcons(final Collection<Trainer> trainers, final boolean runInBackground) {
		if (!this.application.getSettingsRapidClubAdmin().getShareiconsoverweb()) {
			return;
		}
		if (runInBackground) {
	...
	private void runUpdateIcons(final Collection<Trainer> trainers) {
		...
		for (final Trainer trainer : trainers) {
			if (!isLocalIconfileUpToDate(iconInfoList, trainer)) {
				final File target = new File(getLocalIconCache(), trainer.getIdString() + ".jpg");
				URL sourceUrl = null;
				try {
					if (this.iconRepositoryType == ICON_REPOSITORY_TYPE_REMOTE) {
						sourceUrl = application.getWebFileManager()
								.getHttpUrl("trainerIcons/" + trainer.getIdString() + ".jpg", null, null);
						log.fine("downloading file \"" + sourceUrl + "\"");
						application.getWebFileManager()
								.downloadFileHtpp("trainerIcons/" + trainer.getIdString() + ".jpg", target);
					} else {
						final File source = new File(
								this.iconRepositoryPath + File.separator + trainer.getIdString() + ".jpg");
						sourceUrl = source.toURI().toURL();
						log.fine("updating icon from source \"" + source.getAbsolutePath() + "\"");
	...
	protected synchronized Document getIconDoc() {
		if (this.iconDoc == null) {
			try {
				if (this.iconRepositoryType == ICON_REPOSITORY_TYPE_REMOTE) {
					this.iconDoc = this.application.getWebFileManager().downloadDocument("iconlist", this.iconDocPath,
	...
	private Document createIconDoc(RapidBeansRuntimeException ex) {
		...
		if (this.iconRepositoryType == ICON_REPOSITORY_TYPE_REMOTE) {
			iconDoc.setUrl(this.application.getWebFileManager().getFtpUrl(this.iconDocPath, null, null));
		} else {
			try {
	 ...
	 */
	private BufferedImage readScaledLocalFile(final File iconFile) throws IOException {
		...
		if (origImage.getHeight(null) > ICON_DIM || origImage.getWidth(null) > ICON_DIM) {
			final Image tkImage = origImage.getScaledInstance(ICON_DIM, ICON_DIM, Image.SCALE_SMOOTH);
			final BufferedImage scaledImage = new BufferedImage(ICON_DIM, ICON_DIM, BufferedImage.TYPE_INT_RGB);
	 ...
	 * @return false if the local icon file needs a update, true if not
	 */
	...
	private File getLocalIconCache() {
		if (this.localIconCache == null) {
			File iconDir = new File(SettingsAll.getDirname() + File.separator + "trainerIcons");
			if (!iconDir.exists()) {
	...
	public static Trainer createDepartmentIconDummyTrainer(final Club club, final Department department) {
		...
		if (club == null && department == null) {
			departmentIconDummyTrainer = new Trainer(new String[] { "Department", "dummy", "dummy" });
		} else if (club == null) {
			departmentIconDummyTrainer = new Trainer(new String[] { "Department", "dummy", department.getName() });
		} else if (department == null) {
			departmentIconDummyTrainer = new Trainer(new String[] { "Department", club.getName(), "dummy" });
		} else {
			departmentIconDummyTrainer = new Trainer(
					new String[] { "Department", club.getName(), department.getName() });
		}
