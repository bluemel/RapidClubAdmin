...
public class MenuHelper {
	...
	/**
	 * initialize authorization specific menus according to ACLs.
	 */
	public static void updateSpecificMenus() {
		...
		final Set<Department> authDeps = ((ClubadminUser) client.getAuthenticatedUser()).getAuthorizedDepartments();
		...
		if (createNew) {
			...
			for (final Department dep : authDepsSorted) {
				if (findMenuitem(menuTrainingslist, dep.getIdString()) == null && dep.getTrainingdates() != null
		...
		for (Department dep : authDepsSorted) {
			if (findMenuitem(menuOpenCurrent, dep.getIdString()) == null && dep.getTrainingdates() != null
					&& dep.getTrainingdates().size() > 0) {
	...
	public static SubmenuSwing createSubmenu(final SubmenuSwing parentMenu, final String historyBp) {
		final ConfigSubmenu config = new ConfigSubmenu(new String[] { historyBp });
		final SubmenuSwing submenu = new SubmenuSwing(config, ApplicationManager.getApplication(),
	 ...
	 */
	public static void createMenuItem(final SubmenuSwing parentMenu, final String bp, final String dep,
			final Action action) {
		final ConfigMenuItem config = new ConfigMenuItem(new String[] { dep });
		final MenuItemSwing depMenuItem = new MenuItemSwing(config, ApplicationManager.getApplication(),
				"mainwindow.menubar.file.trainingslist");
		...
		depMenuItemWidget.addActionListener(new ActionHandlerActionListener(action));
	...
	private static List<String> getHistorybillingperiodDepartments(final String bp) {
	...
	private static boolean determineNewAllowed() {
		...
		final Set<Department> authnDepartments = ((ClubadminUser) client.getAuthenticatedUser())
				.getAuthorizedDepartments();
		if ((!((User) client.getAuthenticatedUser()).hasRole("SuperAdministrator")) || authnDepartments == null
