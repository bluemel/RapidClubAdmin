...
public class EditorSalaryComponent extends EditorBeanSwing {
	 ...
	 */
	public EditorSalaryComponent(final Application client, final DocumentView docView, final RapidBean bizBean,
			final PropertyCollection newBeanParentColProp) {
		super(client, docView, bizBean, newBeanParentColProp);
	}

	/**
	 * Adds the update of the SalaryComponent's time to the apply action handler.
	 */
	@Override
	public void handleActionApply() {
		super.handleActionApply();
		final EditorPropertySalaryComponentTime propedTime = (EditorPropertySalaryComponentTime) this
				.getPropEditor("time");
		propedTime.updateScTime();
		final EditorPropertySalaryComponentMoney propedMoney = (EditorPropertySalaryComponentMoney) this
				.getPropEditor("money");
