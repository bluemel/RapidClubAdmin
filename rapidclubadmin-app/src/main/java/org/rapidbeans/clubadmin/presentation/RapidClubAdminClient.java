...
public class RapidClubAdminClient extends Application {
	 ...
	 * (relative) application root. E. g. server = www.muenchen-surf.de/bluemel root
	 * = software/rapidbeans/rapidclubadmin/test
	 * 
	 * The server root is given as start argument "-server" if the application is
	 * supposed to run against a server ("web" case - the usual productive use
	 * case). If the application is meant to run locally do not specify this
	 * argument at all. The server root stays null then. ("local" case - the usual
	 * test use case)
	 */
	...
	/**
	 * The application root is relative to the server root in the "web" case e. g.
	 * server = "www.muenchen-surf.de/bluemel" root =
	 * "software/rapidbeans/rapidclubadmin/test" In the local case it is simply an
	 * absolute file system path. e. g. server = null root = -root
	...
	public void start() {
		...
		if (getRunMode() == DataFileType.web) {
			...
			this.trainerIcons = new TrainerIconManager(this, TrainerIconManager.ICON_REPOSITORY_TYPE_REMOTE, null, null,
					null);
		} else {
			...
			if (this.getAuthenticatedUser() != null
				...
				if (!this.getTestMode()) {
					this.messageError(
							...
							this.getCurrentLocale().getStringMessage("error.authorization.title"));
				}
				System.exit(1);
			}
		}
		...
		getMainwindow().show();
		getMainwindow().getFooter()
				.showMessage(getCurrentLocale().getStringMessage("info.clubadmin.started.successfully"));
	...
	public RapidClubAdminSettings getSettingsRapidClubAdmin() {
		...
		if (this.getTestMode()) {
			settings = (Settings) new Document(new File("src/test/resources/testsettings.xml")).getRoot();
		} else {
	...
	private File getLocalmasterdatafile() {
		...
		if (!localMaserdataFile.exists()) {
			throw new RapidBeansRuntimeException(
					"Local masterdata file \"" + localMaserdataFile.getAbsolutePath() + "\" not found.");
		}
	 ...
	 *                   Must be <code>null</code> if a current TrainingsList is to
	 *                   be loaded.
	 * @param department specifies the department for which the TrainingsList
	 *                   document shall be loaded.
	 * 
	...
	public Document loadTrainingslistDocument(final BillingPeriod bp, final Department department) {
		...
		case local:
			...
			if (localCurrentTrainingsDataFile == null) {
				throw new LocalizedException("error.load.file.local.trainingslist.nourl", "error.load.file.local.title",
			...
			if (!localCurrentTrainingsDataFile.exists()) {
				throw new LocalizedException("error.load.file.local.trainingslist.filenotfound",
						"error.load.file.local.title",
						"local file \"" + localCurrentTrainingsDataFile.getAbsolutePath() + "\" not found.",
			...
			if (!localCurrentTrainingsDataFile.canRead()) {
				throw new LocalizedException("error.load.file.local.trainingslist.filenotreadable",
						"error.load.file.local.title",
						"local file \"" + localCurrentTrainingsDataFile.getAbsolutePath() + "\" is not readable.",
						new String[] { localCurrentTrainingsDataFile.getAbsolutePath() });
			}
	...
	/**
	 * Sets the current working department if annotated in the given TrainingsList
	 * document.
	 * 
	...
	public void popupProgramSettings(final String property) {
		...
		if (proped == null) {
			ApplicationManager.getApplication()
					.messageError("property editor for property \"" + property + "\" not found");
		} else {
	...
	/**
	 * Drives the action to save the active document as the current trainings list.
	 * 
	...
	public File getCurrentTrainingsDataFileLocal(final File trainingsDocRoot, final BillingPeriod bp,
		...
		if (!trainingsDocRoot.exists()) {
			throw new RapidBeansRuntimeException(
					"Root folder \"" + trainingsDocRoot.getAbsolutePath() + "\"" + " does not exist.");
		}
		if (!trainingsDocRoot.isDirectory()) {
			throw new RapidBeansRuntimeException(
					"File \"" + trainingsDocRoot.getAbsolutePath() + "\"" + " is not a folder.");
		}
		...
		if (dep == null) {
			...
			if (this.getMasterData().getClubs().size() == 1) {
				if (bp == null) {
					...
					localTrainingsDocFile = new File(trainingsDocRoot.getAbsolutePath() + "/history/" + bp.getIdString()
							+ "/" + dep.getName() + "/" + FILE_NAME_TRAININGS_LIST);
				}
			} else {
				if (bp == null) {
					localTrainingsDocFile = new File(
							trainingsDocRoot.getAbsolutePath() + "/current/" + ((Club) dep.getParentBean()).getName()
									+ "/" + dep.getName() + "/" + FILE_NAME_TRAININGS_LIST);
				} else {
					localTrainingsDocFile = new File(trainingsDocRoot.getAbsolutePath() + "/history/" + bp.getIdString()
