...
public class ToolbarTrainingslistSwing extends ToolbarSwing implements DocumentChangeListener {

	private Document activeDocument = null;

	public ToolbarTrainingslistSwing(final Application client, final MainWindow mainWindow,
		...
		if (authnUser != null) {
			...
			for (final Department dep : authDepsSorted) {
				...
				final ToolbarButtonTrainingslistDepartment button = new ToolbarButtonTrainingslistDepartment(dep, conf,
						client, mainWindow, resourcePath);
	...
	public void update() {
		...
		if (this.activeDocument != null && this.activeDocument != currentlyActiveDocument) {
